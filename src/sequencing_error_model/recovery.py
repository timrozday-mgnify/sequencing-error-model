...
def cigar_mode(templates: list[str], reads: list[Read], mates: list[int], truth: ErrorModelSpec) -> ErrorModelSpec:
    ...
    alphabet, table = truth.quality_alphabet, _tuples(truth, templates, reads, mates)
    error_head = error.fit(table, [c.token for c in truth.error_head], alphabet)
    ...
    lags = [f"q-{i}" for i in range(1, truth.quality_head[0].args[0] + 1)]
    q_table = replace(table, counts=final).marginal(*lags, "pos_start", "pos_end", "mate", "context", "q")
    quality_head = quality.fit(q_table, [c.token for c in truth.quality_head], alphabet)
    return ErrorModelSpec(alphabet, {"mode": "reference", "sources": ["generator-cigar"]}, quality_head, error_head)
